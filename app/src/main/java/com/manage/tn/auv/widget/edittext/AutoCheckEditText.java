package com.manage.tn.auv.widget.edittext;
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        if(!hasFocus){
            if ( this.getText()!= null && this.getText().length() > 0) {
                if (this.getText().length() >= mMinLength && this.getText().length() <= mMaxLength) {
                    boolean isMatch = Check.match(mType, this.getText().toString(), userRegx);
                    changeWarnStatus(!isMatch, mWarningMsg.getMsg());
                } else {
                    changeWarnStatus(true, mWarningMsg.getLengthMsg());
                }
