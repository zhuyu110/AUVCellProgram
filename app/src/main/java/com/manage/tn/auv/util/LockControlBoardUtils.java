...
import com.android_serialport_api.SerialPortUtil;
   ...
   public static LockControlBoardUtils getInstances() {
    ...
    private static void OpenDoor(int arg11, byte[] arg12, int arg13) {
        ...
        if (arg11 == 0) {
            ...
            int v5_1=Integer.parseInt(MyApplication.strMainCellTotal);
            int v6_1=Integer.parseInt(MyApplication.strSecondaryCellTotal);
            LockControlBoardUtils.msleep(v5_1 * 550);
            for (int v7 =1; v7 <= Integer.parseInt(MyApplication.getStrSecondaryCount); ++v7) {
    ...
    public void openSerialPort() {
        SerialPortUtil.getInstances().openSerialPort(MyApplication.PORT_NAME);
    ...
    public static int[] parseCellID(int arg9) {
        int v0=-1;
        int v1=-1;
        int v2=MyApplication.strMainCellTotal.isEmpty() ? 18 : Integer.parseInt(MyApplication.strMainCellTotal);
        int v3=MyApplication.strSecondaryCellTotal.isEmpty() ? 12 : Integer.parseInt(MyApplication.strSecondaryCellTotal);
    ...
    public void getAllDoorStatus(List<AUVBoardCellDevice> devices){
        for(AUVBoardCellDevice dev:devices){
            getDoorStatue(dev.getCellNo());
        }
